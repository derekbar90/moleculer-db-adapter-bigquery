...
const moleculer_1 = tslib_1.__importStar(require("moleculer"));
const _ = require("lodash");
const Promise = require("bluebird");
const bq = knex_1.knex({ client: "pg" });
class BigQueryDbAdapter {
    ...
    init(broker, service) {
        ...
        if (!this.service.schema.bigQuery) {
            ...
            throw new moleculer_1.Errors.ServiceSchemaError("Missing settings definition in schema of service! Please add bigQuery settings object.", null);
        }
    ...
    async connect() {
        var _a;
        this.service.logger.info(`BigQuery connected to projectId: ${(_a = this.bigQueryConfig) === null || _a === void 0 ? void 0 : _a.projectId}`);
    }
    /**
    ...
    async retrieveContext(params) {
        if (Array.isArray(params)) {
            return [
                ...params.map((param) => {
                    const context = param["adapter_private_context"];
                    if (context == null) {
                        throw new moleculer_1.default.Errors.MoleculerError("Unable to retrieve private context, please make sure you apply one via a hook.");
                    }
            ...
            const context = params["adapter_private_context"];
            if (context == null) {
                throw new moleculer_1.default.Errors.MoleculerError("Unable to retrieve private context, please make sure you apply one via a hook.");
            }
    ...
    async findOne(query) {
        ...
        const compiledQuery = bq(`${context === null || context === void 0 ? void 0 : context.tableName}`)
            .where(query)
            .limit(1)
            .toString();
        const results = await this.query(this.formatQuery(compiledQuery), { location: context === null || context === void 0 ? void 0 : context.region });
        //@ts-ignore
    ...
    async findById(context, id) {
        ...
        const compiledQuery = bq(`${parsedContext === null || parsedContext === void 0 ? void 0 : parsedContext.tableName}`)
            .where({
            [primaryKey]: id,
        })
            .limit(1)
            .toString();
        const results = await this.query(this.formatQuery(compiledQuery), { location: parsedContext === null || parsedContext === void 0 ? void 0 : parsedContext.region });
        return results.shift();
    }
    /**
     ...
     * @memberof BigQueryDbAdapter
     */
    async findByIds(context, idList) {
        const parsedContext = await this.retrieveContext(context);
        const primaryKey = await this.bigQueryConfig.getIdKey();
        const compiledQuery = bq(`${parsedContext === null || parsedContext === void 0 ? void 0 : parsedContext.tableName}`)
    ...
    async insert(entity) {
        ...
        const primaryKey = await this.bigQueryConfig.getIdKey();
        const compiledQuery = `
      ${this.formatQuery(bq(`${parsedContext === null || parsedContext === void 0 ? void 0 : parsedContext.tableName}`)
            .insert({
     ...
     * @memberof BigQueryDbAdapter
     */
    async insertMany(entities, opts = { returning: true }) {
        ...
        const primaryKey = await this.bigQueryConfig.getIdKey();
        const compiledQuery = `
      ${this.formatQuery(bq(`${parsedContext.tableName}`).insert(entities).toString())};
      ${this.formatQuery(bq(`${parsedContext.tableName}`)
            .whereIn(primaryKey, entities.map((entity) => entity[primaryKey]))
            .toString())};
    `;
        const results = await this.query(this.formatQuery(compiledQuery), { location: parsedContext === null || parsedContext === void 0 ? void 0 : parsedContext.region });
        return results;
    ...
    async updateMany(where, update) {
      ...
      ${this.formatQuery(bq(`${parsedContext.tableName}`)
            ...
            .update(update)
            .toString())};
      ${this.formatQuery(bq(`${parsedContext.tableName}`).where(where).toString())};
    `;
        const results = await this.query(this.formatQuery(compiledQuery), { location: parsedContext === null || parsedContext === void 0 ? void 0 : parsedContext.region });
        return results;
    }
    ...
    `;
        const compiledQuery = `
      ${this.formatQuery(bq(`${parsedContext.tableName}`).where(where).del().toString())};
    `;
        const entitiesToBeRemoved = await this.query(this.formatQuery(compiledPreQuery), { location: parsedContext === null || parsedContext === void 0 ? void 0 : parsedContext.region });
        await this.query(this.formatQuery(compiledQuery), { location: parsedContext === null || parsedContext === void 0 ? void 0 : parsedContext.region });
    ...
    async migrateIdParamToPrimaryKey(whereObject) {
        const whereObjectCopy = Object.assign({}, whereObject);
    ...
    async createCursor(params = {}, isCounting) {
        ...
        let q = bq(`${context === null || context === void 0 ? void 0 : context.tableName}`);
        if (Object.keys(params || {}).length > 0) {
            // Full-text search
            if (_.isString(params === null || params === void 0 ? void 0 : params.search) && (params === null || params === void 0 ? void 0 : params.search) !== "") {
                let fields = [];
        ...
        const result = await this.query(formattedQuery, { location: context === null || context === void 0 ? void 0 : context.region });
    ...
    async query(query, queryOptions) {
        ...
        const options = Object.assign({ query: formattedQuery, 
            // Location must match that of the dataset(s) referenced in the query.
            location: "US" }, queryOptions);
