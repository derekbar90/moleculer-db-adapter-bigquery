...
class BigQueryDbAdapter {
    ...
    async findOne(query) {
        ...
        const compiledQuery = bq(`${context === null || context === void 0 ? void 0 : context.tableName}.compiled`)
            .where(query)
            .limit(1)
            .toString();
    ...
    async findById(context, id) {
        ...
        const compiledQuery = bq(`${parsedContext === null || parsedContext === void 0 ? void 0 : parsedContext.tableName}.compiled`)
            .where({
            [primaryKey]: id,
        })
            .limit(1)
    ...
    async insert(entity) {
        ...
        const primaryKey = await this.bigQueryConfig.getIdKey();
        const compiledQuery = `
      ${this.formatQuery(bq(`${parsedContext === null || parsedContext === void 0 ? void 0 : parsedContext.tableName}.compiled`)
            .insert({
    ...
    async insertMany(entities, opts = { returning: true }) {
        ...
        const primaryKey = await this.bigQueryConfig.getIdKey();
        const compiledQuery = `
      ${this.formatQuery(bq(`${parsedContext.tableName}.compiled`).insert(entities).toString())};
      ${this.formatQuery(bq(`${parsedContext.tableName}.compiled`)
    ...
    async updateMany(where, update) {
      ...
      ${this.formatQuery(bq(`${parsedContext.tableName}.compiled`)
            ...
            .update(update)
            .toString())};
      ${this.formatQuery(bq(`${parsedContext.tableName}.compiled`).where(where).toString())};
    `;
        const results = await this.query(this.formatQuery(compiledQuery), {});
        return results;
    }
    ...
    `;
        const compiledQuery = `
      ${this.formatQuery(bq(`${parsedContext.tableName}.compiled`).where(where).del().toString())};
    `;
        const entitiesToBeRemoved = await this.query(this.formatQuery(compiledPreQuery), {});
    ...
    async createCursor(params = {}, isCounting) {
        ...
        let q = bq(`${context === null || context === void 0 ? void 0 : context.tableName}.compiled`);
